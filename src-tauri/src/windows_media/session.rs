//! Windows-only WinRT media-session reading (split from windows_media.rs).
...
impl WindowsMediaProvider {
    #[cfg(windows)]
    pub(crate) fn init_session_manager() -> Option<GlobalSystemMediaTransportControlsSessionManager> {
...
#[cfg(test)]
mod tests {
    use crate::windows_media::WindowsMediaProvider;
