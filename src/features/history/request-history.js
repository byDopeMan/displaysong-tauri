 ...
 */
export async function initRequestHistory() {
  loadConfig();
  setupListeners();
  updateUI();
...
function updateUI() {
  // Toggle buttons
  document.querySelectorAll('#history-storage-toggle .toggle-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.storage === storageMode);
  });
 ...
 */
async function checkScopesQuietly() {
 ...
 */
async function checkScopesAndSetup() {
  const invoke = getTauriInvoke();
  if (!invoke) return;
 ...
 */
async function openPlaylistSetup() {
  // First check scopes
...
async function createPlaylist() {
  ...
  const publicCheck = document.getElementById('playlist-public');
  const errorEl = document.getElementById('playlist-setup-error');
 ...
 */
export async function addToHistory(spotifyUri, trackInfo) {
  if (storageMode !== 'spotify' || !playlistConfig?.id) {
    return; // Local storage is handled by SQLite queue
  }
  
  const autoAdd = document.getElementById('playlist-auto-add')?.checked ?? true;
  if (!autoAdd) return;
 ...
 * Get playlist config
 */
export function getPlaylistConfig() {
