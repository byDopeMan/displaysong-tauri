...
interface PlayCallbacks {
  onPlaying?: (durationMs: number, title?: string) => void;
  onEnded?: () => void;
...
const urlCache = new Map<string, StreamInfo>();
...
export function prefetchYouTube(videoId: string): void {
  if (!videoId) return;
  const cached = urlCache.get(videoId);
...
function ensureAudio(): HTMLAudioElement {
  ...
  audio.addEventListener('playing', () => {
    const durMs = (audio && audio.duration && isFinite(audio.duration))
 ...
 */
export async function playYouTube(videoId: string, cb: PlayCallbacks): Promise<void> {
  callbacks = cb || {};
...
export function getYouTubeProgressMs(): number {
  return Math.round((audio?.currentTime || 0) * 1000);
}

function clampVol(v: number): number { return Math.max(0, Math.min(100, v)); }
